...
// Define the Project type, ensuring it matches your Firestore data structure
type Project = {
    id: string;
    title: string;
    ...
    liveUrl: string;
    githubUrl: string;
    tags: string | string[]; // Can be a string from DB or an array
};

const ProjectCard = ({ project }: { project: Project }) => {
    const [ref, isVisible] = useOnScreen({ threshold: 0.2 });

    // Ensure tags are always an array for consistent mapping
...
export const ProjectSection = () => {
    ...
    useEffect(() => {
        ...
        if (!projectsCollectionPath) {
            console.error("Projects collection path is not available.");
            setLoading(false);
            return;
        }
        ...
        return () => unsubscribe();
    }, [isAuthReady, projectsCollectionPath]);
