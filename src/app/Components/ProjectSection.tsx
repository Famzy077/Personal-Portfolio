...
export const ProjectSection = () => {
    ...
    useEffect(() => {
        if (!isAuthReady || !publicProjectsPath || !isSectionVisible) return;
        ...
        return () => unsubscribe();
    }, [isAuthReady, publicProjectsPath, isSectionVisible]);
    ...
    return (
        <section ref={sectionRef as React.RefObject<HTMLDivElement>} id="projects" className="py-20 bg-slate-100 dark:bg-slate-950">
            <div className="mx-auto px-6">
                ...
                <AnimatedSection className="flex justify-center mb-12">
                    <div className="flex space-x-2 bg-slate-200 dark:bg-slate-800 p-1.5 rounded-full">
                        {categories.map(category => (
                            <button key={category} onClick={() => setSelectedCategory(category)} className={`px-4 py-2 text-sm md:text-base font-semibold rounded-full transition-colors ${selectedCategory === category ? 'bg-white dark:bg-slate-700 text-blue-600 dark:dark:text-slate-100 shadow' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-700'}`}>
                                {category}
                            </button>
                        ))}
                    </div>
                ...
                {isSectionVisible && (
                    <div className="grid md:grid-cols-2 lg:mx-20 lg:grid-cols-2 gap-6">
                        {loading ? (
                ...
                {!isSectionVisible && (
                    <div className="text-center text-slate-500 dark:text-slate-400">Scroll down to see my projects!</div>
                )}
            </div>
        </section>
    );
